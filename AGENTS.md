...
Go CLI tool and MCP server for JIRA Cloud. Provides daily summaries, natural language search, AI digest reports, ticket enrichment, weekly status reports, comment summarization, backlog health checks, duplicate detection, full CRUD operations, and Confluence integration (page/blog CRUD, search, analytics, comments, labels). All features are exposed both as CLI commands and 27 MCP tools. Supports stdio and SSE transports, with container deployment for shared multi-client access. Confluence tools support `body_file` parameter for large content that would exceed LLM context limits.
...
- **`internal/mcpserver/`** — MCP server using mcp-go. `tools.go` has core tool handlers (summary, query, digest, enrich, weekly, health, comments, find-similar, and all Confluence tools including create page/blog with `body_file` support). `crud_tools.go` has CRUD tool handlers. `server.go` registers all 27 tools, supports stdio and SSE transports via `Config.Transport`. `store.go` is the in-memory result store with SQLite persistence. `results.go` has per-tool result data structs. `webserver.go` serves the HTML dashboard and `/healthz` health check endpoint.
